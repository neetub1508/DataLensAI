...
export default function LoginPage() {
  ...
  useEffect(() => {
    ...
    if (message === 'registration-success') {
      setSuccessMessage('Registration successful! Please check your email for verification, then sign in below.')
    }
  }, [searchParams])
  ...
  const handleSocialLogin = async (provider: string) => {
    try {
      ...
      if (data.error === 'oauth2_not_configured') {
        // Show user-friendly message about OAuth2 setup
        alert(`Social login with ${provider.toUpperCase()} is not yet configured. Please use email/password login for now, or contact support for OAuth2 setup.`)
      } else {
        // If OAuth2 is properly configured, redirect to the authorization URL
        window.location.href = apiUrl
      }
    } catch (error) {
      console.error('Social login error:', error)
      alert('Social login is currently unavailable. Please use email/password login.')
    }
  }
        ...
        <div className="max-w-md w-full space-y-8">
          <div>
            ...
            <p className="mt-2 text-center text-sm text-gray-600 dark:text-gray-400">
              Sign in to your Data Lens AI account
            </p>
          </div>
